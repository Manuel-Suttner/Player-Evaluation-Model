...
# if you already saved a per-match file, load it, else use df directly
if os.path.exists("role_scored_timeseries.csv"):
    ts_df = pd.read_csv("role_scored_timeseries.csv")
    # your file had both performance_score and learned_role_score
else:
    # fallback if you didn’t save it
    ts_df = df[[
...
target_col = "performance_score"   # you said this is fine
...
for role in ts_df["role_clean"].dropna().unique():
    ...
    for pid in tqdm(players, desc=f"{role} players"):
        ...
        # 5) refit ARIMA on full series to predict next game
        try:
