...
# ======================================================
# ✅ NEW: Fixed Defensive Action Classification
# Tackles: include successful tackles and forced turnovers (failed dribbles/take-ons)
# Interceptions: only when ball progresses forward (real turnovers)
# ======================================================
...
# ======================================================
# Season-17/18 outcome-heavy scoring with minutes control
# Replaces the old "Weighted Performance Score" section
# ======================================================
import numpy as np
import pandas as pd
...
# ---- 2) Percentile scaling inside competition+role for each per90 feature
...
# ---- 3) Minutes reliability factor per player-season
...
# ---- 4) Outcome-heavy game score with team-strength correction already computed earlier
def score_row(r):
...
# ---- 5) Collapse to season-level ranking for 2017/18 Your seasons are identified by season_id. Find the season_id
# for 17/18 automatically by most frequent season in games from 2017-07-01 to 2018-06-30.
...
# ======================================================
# PART 2 — Role-wise supervised scoring (predict next-match performance)
# Replaces the old Autoencoder block
# ======================================================
import numpy as np
import pandas as pd
...
# We will predict the NEXT match's performance_score using current per90 features.
# This is a supervised, role-wise model that yields interpretable weights.

# -----------------------------
# 1) Define feature set (aligned with Part 1)
# -----------------------------
role_features = [
...
# -----------------------------
# 2) Create next-match target per player (temporal shift)
# -----------------------------
...
# -----------------------------
# 3) Train per-role models (temporal split)
# -----------------------------
results_rows = []
coef_rows = []
predictions_all = []
...
# Choose model type: Lasso (sparse, more interpretable) or Ridge (stable)
# We'll try LassoCV first, fall back to RidgeCV if Lasso is too sparse for a role.
def make_lasso():
    return Pipeline([
...
# ----------------------------- 4) Create a per-match learned role score (for every row) Score = predicted
# next-match performance given current features. We fit on ALL available data per role (temporally valid use
# later: use only for forecasting from current match). -----------------------------
learned_scores_list = []
...
for role in roles:
    ...
    # Simple heuristic: if Lasso worked reasonably (|R2| not terrible) keep it; else use Ridge.
    # For stability, use Ridge here to produce smooth scores unless Lasso had R2 >= 0.1
...
# -----------------------------
# 5) Quick sanity check rankings (optional, prints only)
# -----------------------------
...
# ======================================================
# PART 3 — XGBoost Next-5-Match Forecast
# ======================================================
...
print("\n🚀 Training XGBoost Next-5-Match Forecast Models...")

# Load the learned role dataset
learned_df = pd.read_csv("role_scored_timeseries.csv")
learned_df["game_date"] = pd.to_datetime(learned_df["game_date"])
learned_df = learned_df.sort_values(["role_clean", "player_id", "game_date"])
...
target_col = "learned_role_score"

# Fill missing data
learned_df[feature_cols] = learned_df[feature_cols].fillna(0)
learned_df[target_col] = learned_df[target_col].fillna(learned_df[target_col].mean())

# Train role-wise models
results = []
forecasts = []

roles = learned_df["role_clean"].dropna().unique()

for role in roles:
    role_df = learned_df[learned_df["role_clean"] == role].copy().sort_values("game_date")

    if len(role_df) < 60:
        print(f"⚠️ Skipping role '{role}' (not enough samples).")
        continue

    # Temporal split (80/20)
    split_idx = int(len(role_df) * 0.8)
    train_df = role_df.iloc[:split_idx]
    test_df = role_df.iloc[split_idx:]
    ...
    # Forecast next 5 matches (using most recent data)
    latest = role_df.groupby("player_id").tail(1).copy()
    X_future = scaler.transform(latest[feature_cols].values)
    latest["xgb_forecast_next5"] = model.predict(X_future)
    forecasts.append(latest[["player_id", "shortName", "role_clean", "xgb_forecast_next5"]])

# Consolidate and save
eval_df = pd.DataFrame(results).sort_values("role")
forecast_df = pd.concat(forecasts, ignore_index=True)

eval_df.to_csv("xgboost_forecast_eval.csv", index=False)
forecast_df.to_csv("xgboost_next5_forecasts.csv", index=False)
print("✅ Saved xgboost_forecast_eval.csv & xgboost_next5_forecasts.csv")

# ======================================================
# Identify Top Performers and Trending Players
# ======================================================
agg_actual = (
    learned_df.groupby(["player_id", "shortName", "role_clean"], as_index=False)["learned_role_score"]
    .mean()
    .rename(columns={"learned_role_score": "avg_actual_score"})
)

merged = forecast_df.merge(agg_actual, on=["player_id", "shortName", "role_clean"], how="left")
merged["forecast_delta"] = merged["xgb_forecast_next5"] - merged["avg_actual_score"]

# Top future performers
top_future = (
    merged.sort_values(["role_clean", "xgb_forecast_next5"], ascending=[True, False])
    .groupby("role_clean")
    .head(15)
)
top_future.to_csv("top15_future_forecast.csv", index=False)

# Top past performers
top_actual = (
    agg_actual.sort_values(["role_clean", "avg_actual_score"], ascending=[True, False])
    .groupby("role_clean")
    .head(15)
)
top_actual.to_csv("top15_actual_performers.csv", index=False)

# Rising and declining players
rising = merged.sort_values("forecast_delta", ascending=False).head(15)
declining = merged.sort_values("forecast_delta", ascending=True).head(15)

rising.to_csv("rising_players.csv", index=False)
declining.to_csv("declining_players.csv", index=False)

print("✅ Saved: top15_actual_performers.csv")
print("✅ Saved: top15_future_forecast.csv")
print("✅ Saved rising_players.csv & declining_players.csv")
print("\n🎯 XGBoost forecasting model complete!")
