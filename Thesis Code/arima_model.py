...
# PART 3 — Simple ARIMA Forecasting
import numpy as np
import pandas as pd
...
# --- Compute average RMSE and MAE per competition and role for ARIMA ---
print("\n📊 Computing RMSE and MAE averages by competition and role (ARIMA)...")

eval_arima = eval_df.copy()
eval_arima["model_name"] = "ARIMA"

try:
    comp_map = ts_df[["player_id", "role_clean", "competition_name"]].drop_duplicates()
    eval_arima = eval_arima.merge(comp_map, on="player_id", how="left")
except Exception as e:
    print("⚠️ Could not merge competition info:", e)
    eval_arima["competition_name"] = "Unknown"

eval_by_comp_role = (
    eval_arima.groupby(["model_name", "role", "competition_name"], as_index=False)
    .agg(avg_RMSE=("RMSE", "mean"),
         avg_MAE=("MAE", "mean"))
)
eval_by_comp_role.to_csv("arima_eval_by_competition_and_role.csv", index=False)
print("✅ Saved ARIMA evaluation summary to 'arima_eval_by_competition_and_role.csv'")
