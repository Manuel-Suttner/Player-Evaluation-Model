...
# === Load Dataset ===
df = pd.read_csv("role_scored_timeseries.csv", parse_dates=["game_date"])
df = df.dropna(subset=["performance_score", "learned_role_score", "role_clean"])
df["role_clean"] = df["role_clean"].astype(str).str.lower()

# --- Step 1: Compute per-player performance dynamics ---
def compute_trend(subdf):
    if len(subdf) < 3:
        return 0
    x = (subdf["game_date"] - subdf["game_date"].min()).dt.days.values
    y = subdf["performance_score"].values
    return linregress(x, y).slope

trend = df.groupby("player_id").apply(compute_trend).rename("trend_slope")

agg = (
    df.groupby(["player_id", "shortName", "role_clean"], as_index=False)
    .agg(
        mean_perf=("performance_score", "mean"),
        std_perf=("performance_score", "std"),
        mean_pred=("learned_role_score", "mean"),
        std_pred=("learned_role_score", "std"),
        mean_error=("performance_score", lambda x: np.mean(x - df.loc[x.index, "learned_role_score"])),
        rmse_error=("performance_score", lambda x: np.sqrt(np.mean((x - df.loc[x.index, "learned_role_score"]) ** 2))),
        n_matches=("game_id", "count")
    )
    .merge(trend, on="player_id", how="left")
    .fillna(0)
)

# --- Step 2: Include average per-90 metrics (technical profile) ---
per90_cols = [c for c in df.columns if c.endswith("_per90")]
player_features = df.groupby("player_id")[per90_cols].mean().reset_index()
agg = agg.merge(player_features, on="player_id", how="left").fillna(0)

# --- Step 3: Role-specific clustering ---
all_clusters = []
for role, sub in agg.groupby("role_clean"):
    print(f"\n🔹 Clustering role: {role.upper()} (n={len(sub)})")

    # Combine dynamic + technical features
    dyn_features = ["mean_perf", "std_perf", "rmse_error", "mean_error", "trend_slope"]
    role_features = [f for f in per90_cols if sub[f].nunique() > 1]
    features = dyn_features + role_features

    X = StandardScaler().fit_transform(sub[features])

    # Try 5 clusters by default
    kmeans = KMeans(n_clusters=5, random_state=42)
    sub["cluster"] = kmeans.fit_predict(X)
    ...
    summary = sub.groupby("cluster")[dyn_features].mean().round(2)
    print(summary)

    # Simplified and cleaner cluster labeling (no "Volatile" category)
    labels = {}
    for cid in sub["cluster"].unique():
        row = summary.loc[cid]
        if row["mean_error"] > 1.5:
            labels[cid] = "Overperforming Improvers"
        elif row["mean_error"] < -3:
            labels[cid] = "Underperforming Decliners"
        elif row["mean_perf"] > 18 and row["std_perf"] < 4:
            labels[cid] = "Reliable Anchors"
        else:
            labels[cid] = "Steady Performers"

    sub["cluster_label"] = sub["cluster"].map(labels)

    # --- Save role-specific visualization ---
    plt.figure(figsize=(8, 6))
    sns.scatterplot(data=sub, x="pc1", y="pc2", hue="cluster_label", palette="Set2")
    plt.title(f"{role.title()} Clusters based on Dynamics & Technical Profile")
    plt.legend(bbox_to_anchor=(1.05, 1), loc='upper left')
    plt.tight_layout()
    plt.savefig(f"clusters_{role.lower()}.png", dpi=300)
    plt.close()
...
final_clusters.to_csv("player_role_clusters.csv", index=False)
print("\n✅ Saved: player_role_clusters.csv")

# Summary: number of players per role + label
summary = final_clusters.groupby(["role_clean", "cluster_label"]).size().reset_index(name="count")
