...
# ======================================================
# PART 3 — Simple ARIMA Forecasting (no GRU, no hybrid)
# ======================================================
...
# -----------------------------------------
if "learned_df" in globals() and isinstance(learned_df, pd.DataFrame):
    ts_df = learned_df.copy()
else:
    ts_df = pd.read_csv("role_scored_timeseries.csv")

ts_df["game_date"] = pd.to_datetime(ts_df["game_date"], errors="coerce")
ts_df = ts_df.dropna(subset=["game_date"])
ts_df = ts_df.sort_values(["role_clean", "player_id", "game_date"])

target_col = "learned_role_score"  # or "performance_score"

# -----------------------------------------
# Helper functions
# -----------------------------------------
def series_from_player(frame):
    s = (frame
         .set_index("game_date")[target_col]
         .asfreq("7D")        # weekly frequency
         .interpolate())
    return s
...
CANDIDATE_ORDERS = [(0,1,1), (1,1,0), (1,0,0)]

# -----------------------------------------
# Run ARIMA forecasts
# -----------------------------------------
eval_rows, forecast_rows = [], []
roles = ts_df["role_clean"].dropna().unique()
min_points = 12

for role in roles:
    role_df = ts_df[ts_df["role_clean"] == role]
    for pid in tqdm(role_df["player_id"].unique(), desc=f"{role} players"):
        pframe = role_df[role_df["player_id"] == pid][["shortName", "game_date", target_col]].dropna()
        if len(pframe) < min_points:
            continue

        s = series_from_player(pframe)
        ...
        for order in CANDIDATE_ORDERS:
            try:
                ...
                rmse, mae = evaluate(test.values, preds.values)
                if rmse < best_rmse:
                    best_rmse = rmse
                    best_order = order
                    best_fit = fit
        ...
        eval_rows.append({
            "role": role,
            "player_id": pid,
            "player": pframe["shortName"].iloc[-1],
            "n_obs": n,
        ...
        # Next-match forecast using full series
        try:
            full_fit = ARIMA(s, order=best_order).fit()
            next_val = float(full_fit.forecast(steps=1).values[0])
        except Exception:
            next_val = float(s.values[-1])
...
eval_df.to_csv("arima_forecast_eval.csv", index=False)
fc_df.to_csv("arima_next_forecasts.csv", index=False)

print("\n✅ Simple ARIMA forecasting complete.")
print(f"Models evaluated: {len(eval_df)} | Forecasts generated: {len(fc_df)}")

# Top 15 by forecast
if not fc_df.empty:
    top15 = (fc_df.sort_values(["role", "arima_next_forecast"], ascending=[True, False])
             .groupby("role")
             .head(15)
             .reset_index(drop=True))
    top15.to_csv("top15_arima_forecast.csv", index=False)
    print("\n🏆 Top 15 Players per Role by ARIMA Forecast:")
    for role in top15["role"].unique():
        sub = top15[top15["role"] == role][["player", "arima_next_forecast"]]
        print(f"\n⚽ {role.upper()} — Top 15")
